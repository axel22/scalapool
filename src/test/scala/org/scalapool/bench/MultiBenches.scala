...
object MultiHeapOld extends MultiMain {
  
  val array = new Array[BigFoo](2 << 22)
  
  class HeapThread(idx: Int, sz: Int) extends Thread {
    var f: BigFoo = null
    override def run() {
      var i = 0
      var pos = idx * 4096
  ...
  def run() {
    ...
    val threads = for (idx <- 0 until par) yield new HeapThread(idx, sz)
