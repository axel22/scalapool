...
abstract class Acquirable[R >: Null <: Acquirable[R]] extends JAcquirable with Poolable[R] {
   ...
   */
  private[scalapool] def reallocateInit() {
  ...
  private def disposeToMemoryPool() {
    if (_memory_pool ne null) _memory_pool.dispose(repr)
  }
...
object Acquirable {
  private[scalapool] val statemask = 0xc0000000
  private[scalapool] val countmask = 0x3fffffff
}
